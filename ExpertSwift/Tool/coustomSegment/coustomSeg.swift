...
//  ExpertSwift
//
//  Created by jyyl on 2017/12/18.
//  Copyright © 2017年 jyyl. All rights reserved.
//

import UIKit

enum coustomSegTitleStyle {
    case normal             //默认
    case gradient           //渐变颜色
    case transfrom          //放大
}

enum coustomSegIndicatorStyle {
    case normal             //常规
    case followText         //跟随文本长度
    case stretch            //伸缩  默认
}

class coustomSeg: UIView{
    ...
    //下标宽度
    var indicatorWidth:CGFloat = 30 { //coustomSegIndicatorStyle 为normal时有效
        didSet{
            setNeedsLayout()
        }
    }
    ...
    /// 标题样式
    var titleStyle:coustomSegTitleStyle = .normal {
        didSet{
            setNeedsLayout()
        }
    }
    // 下标样式
    var indicatorStyle:coustomSegIndicatorStyle = .normal {
        didSet{
            setNeedsLayout()
        }
    }
    //伸缩动画的偏移量 在indicatorStyle = stretch时生效
    ...
    //bodyScrollView的父视图，默认为coustomSeg的父视图
    ...
    lazy var bodyScrollView:UIScrollView = {
        ...
        bodyScrollView.backgroundColor = UIColor.blue
...
extension coustomSeg:UIScrollViewDelegate{
